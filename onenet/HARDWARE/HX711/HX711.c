...
/* HX711标准参数定义 */
#define HX711_GAP_VALUE     423.0f      /* 标准系数 - 根据实际砝码调整 */

/* 全局变量定义 */
uint32_t hx711_buffer = 0;              /* HX711原始数据缓冲 */
uint32_t hx711_maopi = 0;               /* 毛皮(零点)值 */
int32_t hx711_weight = 0;               /* 实际重量(g) */
 ...
 * @param 无
 * @retval 无
 * @note 安全使用STM32F10x标准库函数，不使用内存直接操作
 */
void hx711_init(void)
{
    ...
    /* PB3/PB4默认被JTAG占用，关闭JTAG以保护SWD，否则HX711信号无法驱动 */
 ...
 * @note 放大128倍，参考 STM32F103_TEST_HX711项目
 */
uint32_t hx711_read(void)
{
    ...
    GPIO_SetBits(GPIOB, GPIO_Pin_3);            /* SCK拉高 */
    delay_us(2);
    count = count ^ 0x800000;         /* 根据数据手册要求取反 */
    GPIO_ResetBits(GPIOB, GPIO_Pin_3);     /* SCK拉低 */
    delay_us(2);

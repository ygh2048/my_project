...
//SYSCLK:系统时钟
//注意：由于 tick.c 已经配置了 SysTick，这里只初始化倍乘数
void delay_init()
{
	...
	// 注意：不配置 SysTick，因为 tick.c 已经配置了 SysTick 用于 1ms 中断
#endif
}								    
...
//延时nus
//nus为要延时的us数.
//注意：由于 tick.c 已经启用了 SysTick 中断，这里使用读取计数器的方式实现延时
void delay_us(u32 nus)
{		
	u32 ticks;
	u32 told,tnow,tcnt=0;
...
//延时nms
//注意：由于 tick.c 已经启用了 SysTick 中断，这里使用 delay_us 实现
void delay_ms(u16 nms)
{	 		  	  
	u32 i;

...
# 开始录制视频的后台进程
#ffmpeg -f video4linux2 -i /dev/video0 "$VIDEO_FILE" &
ffmpeg -f video4linux2 -s 640*480 -r 30 -i /dev/video0 "$VIDEO_FILE" &
#raspivid -o "$VIDEO_FILE" -t 0 &
VIDEO_PID=$!

# 开始捕获数据包的后台进程
tcpdump -i wlan0 dst port 5500 -vv -w "$DUMP_FILE" -c 70 &
TCPDUMP_PID=$!

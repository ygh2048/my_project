...
void NCLink_Data_Prase_Prepare_gh(uint8_t data)
{
	  static uint8_t buf[100];
	  ...
	  else if(state==3&&data==NCLink_End[0])//帧尾0
		{
				state = 4;
				buf[2+data_cnt++]=data;
		}
	   else if(state==4&&data==NCLink_End[1])//帧尾1
		{
				state = 0;
				buf[2+data_cnt]=data;
				process_gh(buf,data_cnt+3);//数据解析
		}
	  else state = 0;
}

void process_gh(uint8_t *data_buf,uint8_t num)
{
	...
	unpack_overflag=1;
    Byte2Float(data_buf,2, &pos_x_3d);
    Byte2Float(data_buf,6, &pos_y_3d);
    Byte2Float(data_buf,10,&pos_z_3d);
    q0 = ((int16_t)(*(data_buf+14)<<8)|*(data_buf+15));
    q1 = ((int16_t)(*(data_buf+16)<<8)|*(data_buf+17));
    q2 = ((int16_t)(*(data_buf+18)<<8)|*(data_buf+19));
    q3 = ((int16_t)(*(data_buf+20)<<8)|*(data_buf+21));
}
//FF FC F1 A1 F1 F1 F1 F1 F1 F1 F1 F1 F1 F1 F1 A1 A1 A1 A1 1F A1 F1 A1 A2
...
void NCLink_Data_Prase_Process(uint8_t *data_buf,uint8_t num)//飞控数据解析进程
{
  ...
  if(!(sum==*(data_buf+num-3)))    																					return;//判断sum
    ...
    else if(*(data_buf+2)==0X17)
    {
        Byte2Float(data_buf,4, &pos_x_3d);
        Byte2Float(data_buf,8, &pos_y_3d);
        Byte2Float(data_buf,12,&pos_z_3d);
        q0 = 0.0001f*((int16_t)(*(data_buf+16)<<8)|*(data_buf+17));
        q1 = 0.0001f*((int16_t)(*(data_buf+18)<<8)|*(data_buf+19));
        q2 = 0.0001f*((int16_t)(*(data_buf+20)<<8)|*(data_buf+21));
        q3 = 0.0001f*((int16_t)(*(data_buf+22)<<8)|*(data_buf+23));
    }
...
void useget(void)
{
	...
	mydata.q2=q2;
	mydata.q3=q3;
//	mydata.x=(mydata.x>150)?150:mydata.x;
//	mydata.x=(mydata.y>100)?100:mydata.y;
}

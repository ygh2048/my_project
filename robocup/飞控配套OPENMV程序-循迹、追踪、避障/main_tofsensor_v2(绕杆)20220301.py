...
#sensor.set_auto_gain(False)         #在#在颜色追踪时，关闭自动增益颜色追踪时，关闭自动增益
...
def found_line():
    ...
    target.img_height=IMAGE_HEIGHT
    pixels_max=0
    singleline.state = img.get_regression([thresholds],x_stride=2,y_stride=2,pixels_threshold=1,robust = True)
    if(singleline.state):
...
thresholds_red_pole=(10, 75, 50, 100, 10, 80)#(10, 75, 30, 100, 10, 80)
def opv_find_color_pole():
    target.flag=0
    if (ctr.work_mode&0x05)!=0:
        ...
        target.img_height=IMAGE_HEIGHT
        pixels_max=0
        for b in img.find_blobs([thresholds_red_pole],pixels_threshold=5,merge=True,margin=50):
            img.draw_rectangle(b[0:4])#圈出搜索到的目标
            if pixels_max<b.pixels():
                pixels_max=b.pixels()
                target.x = b.cx()
...
#    print(sensor.get_gain_db(),sensor.get_rgb_gain_db(),sensor.get_exposure_us())

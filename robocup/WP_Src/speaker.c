...
void ConfigureUART_Speaker(unsigned long bound)//串口4初始化
{
  ...
  IntPrioritySet(INT_UART4, USER_INT2);//USER_INT5
} 
...
void ConfigureUART_Speaker(unsigned long bound)//串口4初始化
{
  ...
  IntPrioritySet(INT_UART5, USER_INT2);//USER_INT5
} 
...
uint8_t Head[2]={0xFF,0xFC};//数据帧头
uint8_t End[2] ={0xA1,0xA2};//数据帧尾
void speaker_gh(float x,float y,float z,int16_t q0,int16_t q1,int16_t q2,int16_t q3)
{
        ...
        Speaker_Send(sendbuf,len); //发送帧配置
}

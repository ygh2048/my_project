...
void ConfigureUART1(unsigned long bound)//串口1初始化
{
  ...
  IntPrioritySet(INT_UART1, USER_INT3);//USER_INT5
}
...
void Vcan_Send(void)//山外地面站发送
{
  static float DataBuf[8];	
	DataBuf[0]=WP_Sensor.baro_altitude;
  DataBuf[1]=GD_Distance;
  DataBuf[2]=NamelessQuad.Position[_UP];
  DataBuf[3]=NamelessQuad.Speed[_UP];
  DataBuf[4]=NamelessQuad.Acceleration[_UP];
  DataBuf[5]=0;
  DataBuf[6]=0;
  DataBuf[7]=0;

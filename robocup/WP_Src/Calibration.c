...
void Calibrationer(void)
{
  ...
  for(i=0;i<6;i++)
  {
    Test_Calibration[i].x *=RAW_TO_G;
    Test_Calibration[i].y *=RAW_TO_G;
    Test_Calibration[i].z *=RAW_TO_G;
  }
...
void Horizontal_Calibration_Check(void)
{
             ...
             &&Controler_State==Lock_Controler)//必须为上锁状态才可以进入校准模式
  {
    ...
    //Hor_Accel_Offset[2]=(acce_sample_sum[2]/num_samples-AcceMax_1G)*RAW_TO_G; //保存对应面的加速度计量
		
		Hor_Accel_Offset[2]=(acce_sample_sum[2]/num_samples)*RAW_TO_G-safe_sqrt(sq2(GRAVITY_MSS)-sq2(Hor_Accel_Offset[0])-sq2(Hor_Accel_Offset[1]));
		
    
    Horizontal_Calibration_Makesure_Cnt=0;
		WriteFlashParameter(HOR_CAL_ACCEL_X,Hor_Accel_Offset[0],&Flight_Params);
		WriteFlashParameter(HOR_CAL_ACCEL_Y,Hor_Accel_Offset[1],&Flight_Params);
		WriteFlashParameter(HOR_CAL_ACCEL_Z,Hor_Accel_Offset[2],&Flight_Params);
		
		
    WriteFlashParameter(PITCH_OFFSET,Pitch_Offset,&Flight_Params);
    WriteFlashParameter(ROLL_OFFSET,Roll_Offset,&Flight_Params);
...
void Headless_Mode_Calibration_Check(void)
{
  ...
  {    
    ...
    Horizontal_Calibration_Makesure_Cnt=0;
    WriteFlashParameter(HEADLESS_MODE_YAW,Headless_Mode_Yaw,&Flight_Params);
    Bling_Set(&Light_1,500,100,0.2,0,GPIO_PORTF_BASE,GPIO_PIN_1,0);
  }
}
...
void Other_Parameter_Default(void)
{
	...
	WriteFlashParameter(TARGET_HEIGHT,Target_Height,&Flight_Params);
	WriteFlashParameter(SAFE_HEIGHT,Safe_Height,&Flight_Params);
	WriteFlashParameter(SAFE_VBAT,Safe_Vbat,&Flight_Params);
	WriteFlashParameter(MAX_HEIGHT,Max_Height,&Flight_Params);
	WriteFlashParameter(MAX_RADIUS,Max_Radius,&Flight_Params);
	WriteFlashParameter(MAX_UPVEL,Max_Upvel,&Flight_Params);
	WriteFlashParameter(MAX_DOWNVEL,Max_Downvel,&Flight_Params);
	WriteFlashParameter(MAX_HORVEL,Max_Horvel,&Flight_Params);
	WriteFlashParameter(RESERVED_UART_FUNCTION,Reserved_Uart,&Flight_Params);
	WriteFlashParameter(NEAR_GROUND_HEIGHT,Near_Ground_Height,&Flight_Params);
	WriteFlashParameter(UART2_FUNCTION,Uart2_Mode,&Flight_Params);
	WriteFlashParameter(AVOID_OBSTACLE,Avoid_Obstacle,&Flight_Params);
}

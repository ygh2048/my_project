...
void OpticalFlow_Init()
{
  ...
  //根据不同的光流配置，设置对应长度的数据缓冲区	
	...
	else if(Optical_Type_Present==3)//lidar or T265
	{
...
void opticalflow_pretreat(void)
{
	...
	gyro_filter_data.x=LPButterworth(WP_AHRS.Roll_Gyro,&Buffer_OpticalFlow_Gyro[0],&OpticalFlow_Gyro_Parameter)/57.3f;//陀螺仪相位同步角速度
	gyro_filter_data.y=LPButterworth(WP_AHRS.Pitch_Gyro,&Buffer_OpticalFlow_Gyro[1],&OpticalFlow_Gyro_Parameter)/57.3f;//陀螺仪相位同步角速度
...
void Optflow_Statemachine(void)
{
  if(OpticalFlow_Is_Work==1)//若初始化时，存在光流外设
  {
		switch(Optical_Type_Present)
		{
			...
			default:OpticalFlow_INS_CF(NamelessQuad.Position[_UP],SINS_Accel_Body,opt_gyro_filter_data);break;	
		}
  }
}
...
SINS OpticalFlow_SINS,VIO_SINS;
...
void  VIO_SLAM_INS_CF(void)
{
  ...
  { 
	...
	acc.x= Origion_NamelessQuad.Acceleration[_EAST];//惯导加速度沿载体横滚,机头左侧为正
	acc.y= Origion_NamelessQuad.Acceleration[_NORTH];//惯导加速度沿载体机头,机头前向为正

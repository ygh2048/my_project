...
void overturn_check(void)//倾覆强制停机检测，放在5ms执行一次任务中
{
	...
	else 
	{
		...
		//RGB灯闪烁报警
		Bling_Set(&Light_1,3000,1000,0.5,0,GPIO_PORTF_BASE,GPIO_PIN_1,1);//蓝色
		Bling_Set(&Light_2,3000,1000,0.5,0,GPIO_PORTF_BASE,GPIO_PIN_2,1);//红色
		Bling_Set(&Light_3,3000,1000,0.5,0,GPIO_PORTF_BASE,GPIO_PIN_3,1);//绿色	
	}
#endif
}

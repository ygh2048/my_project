...

{
	...
	/* Insert l */
	msg[(num_blocks * 64) - 1] = (unsigned char)(l        & 0xff);
	msg[(num_blocks * 64) - 2] = (unsigned char)((l >> 8)  & 0xff);
	msg[(num_blocks * 64) - 3] = (unsigned char)((l >> 16) & 0xff);
	msg[(num_blocks * 64) - 4] = (unsigned char)((l >> 24) & 0xff);
	...
	for(i = 0; i < num_blocks; i++)
	{
		...
		for(t = 0; t < 80; t++)
		{
			if(t < 16)
			{
				w[t]  = (256 * 256 * 256) * msg[(i * 64) + (t * 4)];
				w[t] += (256 * 256) * msg[(i * 64) + (t * 4) + 1];
				w[t] += (256) * msg[(i * 64) + (t * 4) + 2];
				w[t] +=                 msg[(i * 64) + (t * 4) + 3];
			}
...

{
	...
	/* 用于 SHA1 处理的临时缓冲,避免修改原数据 */
	static unsigned char temp_msg_buf[MAX_MESSAGE_LENGTH + 128];
	int i;
	...
	if(key_length != b)     /* Step 1 */
	{
		...
		if(key_length > b)
		{
			/* Step 2: 先复制 key 到临时缓冲,再用 SHA1 处理 */
			memcpy(temp_msg_buf, key, key_length);
			sha1(temp_msg_buf, key_length, digest);
...
#endif
	/* Step 6: 复制到临时缓冲,再进行 SHA1 处理 */
	memcpy(temp_msg_buf, step5data, data_length + b);
	sha1(temp_msg_buf, data_length + b, digest);
#ifdef HMAC_DEBUG
...
#endif
	/* Step 9: 复制到临时缓冲,再进行最后的 SHA1 处理 */
	memcpy(temp_msg_buf, step8data, b + 20);
	sha1(temp_msg_buf, b + 20, digest);
#ifdef HMAC_DEBUG
